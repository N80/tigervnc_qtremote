...
PollingManager::PollingManager(View *view, const Image *image,
                               ImageFactory &factory,
                               int offsetLeft, int offsetTop)
  : m_view(view),
    m_image(image),
    m_bytesPerPixel(image->bits_per_pixel / 8),
    m_offsetLeft(offsetLeft),
    m_offsetTop(offsetTop),
    m_width(image->width),
    m_height(image->height),
    m_widthTiles((image->width + 31) / 32),
    m_heightTiles((image->height + 31) / 32),
    m_numTiles(((image->width + 31) / 32) *
               ((image->height + 31) / 32)),
    m_pollingStep(0)
{
  ...
  // primary image.
  m_rowImage = factory.newImage(m_view, m_width, 1);
  m_columnImage = factory.newImage(m_view, 1, m_height);
